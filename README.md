...
- **RobustFlow 式鲁棒生成**（多样本、校验筛选、择优与修复，降低同义改写导致的结果跳变）
- **可选评测工具链**（Python：`tools/` 下复用 RobustFlow `graph_evaluator` 与消融作图）

完整改动与论文/仓库对应关系见 **[ROBUSTFLOW_INTEGRATION.md](./ROBUSTFLOW_INTEGRATION.md)**。
...
在仓库中本项目位于 `MWGL-v2/MWGL`（以下路径请按你的克隆位置调整）：
...
- `http://localhost:3001`（或 `.env` 中 `PORT` 指定端口）
...
**鲁棒生成（默认开启，与 RobustFlow 推理期策略对齐；详见 [ROBUSTFLOW_INTEGRATION.md](./ROBUSTFLOW_INTEGRATION.md)）**

- `MWGL_ROBUST`：设为 `0` / `false` 关闭鲁棒管线，仅单次调用模型
- `MWGL_ROBUST_SAMPLES`：并行采样次数（默认 `3`）
- `MWGL_ROBUST_REPAIR_MAX`：校验失败后的修复轮数上限（默认 `2`）
- `MWGL_ROBUST_ENSEMBLE`：多个合法候选时是否再调模型做评审择优（`0` 关闭）

**Python 评测脚本（可选）**

- 句向量模型下载默认使用镜像：`eval_mwgl_robustness.py` 内已 `setdefault HF_ENDPOINT=https://hf-mirror.com`；亦可自行 `export HF_ENDPOINT=...`

### 2.1 鲁棒性评测与消融（简要）

- **单独克隆本仓库**（不含上级 RobustFlow 目录）时，请设置 `ROBUSTFLOW_ROOT` 指向你本地的 [RobustFlow](https://github.com/DEFENSE-SEU/RobustFlow) 根目录，否则无法导入 `graph_evaluator`。
- 本地基准数据：`data/mwgl_robustness_benchmark.jsonl`，说明见 `data/DATASET_SOURCES.txt`
- 仅测提示嵌入 / 字面指标（不调生成 API）：

  `cd tools && python eval_mwgl_robustness.py --prompt-metrics-only`

- 对比「单次生成」与「鲁棒管线」并出 CSV + 图（需已 `npm start`）：

  `cd tools && python mwgl_robustness_ablation.py --base-url http://127.0.0.1:3001`

- 输出目录：`data/reports/`、`data/reports/visual/`；图中中文异常时可加 `--plot-font /本机/字体文件.otf`
...
补充：当前实现区分“连接时即时拦截”和“最终导出校验”两层。  
其中，明显非法连线会在连接时直接阻止；全图一致性约束在导出时统一检查。
...
以下规则在“保存连线”或“Shift 拖线创建连线”时立即生效：

- 禁止连到 `start`
- 禁止从 `success` / `failure` 连出边
- 禁止自环（`from === to`）
- 禁止新增后形成有向环（保持 DAG）
- `loop_start` 必须且仅能有 1 条出边（超出时直接阻止）

说明：这部分用于避免用户在编辑阶段进入必失败状态。
...
- 手动编辑边时：在点击“保存连线”后校验并提示
- Shift 拖线创建边时：会先自动填充默认标签（如 `是/否/条件N`）再校验
...
- 编辑态（画布操作）：
  - 允许临时不满足全图约束（例如尚未连通、终态暂未接入）
  - 但对明显非法连线会即时拦截（如连到 `start`、从终态连出、成环）
  - 目标是提升搭图自由度
...
- `prompt`：必填，自然语言需求。
- `robust`：可选。`true` / `false` 会**覆盖**环境变量 `MWGL_ROBUST`，用于消融或临时切换「单次生成 vs 鲁棒管线」；不传则按 `.env` 中 `MWGL_ROBUST` 行为。
...
```json
{
  ...
  "robust": { "samples": 3, "stages": [...] }
}
```

- `content`：工作流 JSON 字符串（合法时可能为已 `normalize` 的 JSON）。
- `robust`：鲁棒模式下的阶段摘要；单次生成时为 `{ "mode": "single" }`。
...
- 想省 API 调用费用：将 `MWGL_ROBUST_SAMPLES` 调小或设 `MWGL_ROBUST=0`；评测消融仍可用请求体 `"robust": false/true` 分别打两种模式。
- 评测脚本无法下载句向量模型：使用 `HF_ENDPOINT` 镜像、或 `--lexical-only`、或 `--local-files-only` + 本地模型目录（见 `tools/eval_mwgl_robustness.py --help`）。
- 消融图里中文乱码：使用 `python mwgl_robustness_ablation.py ... --plot-font /path/to/NotoSansSC-Regular.otf`，或运行 `tools/smoke_chinese_chart.py` 自检字体。
